...
if __name__ == "calendar":
    _stdlib_calendar_path = Path(sysconfig.get_paths()["stdlib"]) / "calendar.py"
    exec(compile(_stdlib_calendar_path.read_text(), str(_stdlib_calendar_path), "exec"))
else:
    ...
    class PukRumiaPickupCalendar(CalendarEntity):
        ...
        def _build_event(self, pickup_date: date) -> CalendarEvent:
            ...
            return CalendarEvent(
                ...
                summary=f"Odbiór: {self._bin.name}",
                description=f"Rodzaj odpadu: {self._bin.waste_type}",
            )
